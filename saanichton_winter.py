...
# Canadian meteorological websites suggest that the Greater Victoria area experiences its highest rainfall
# in October and November

saanichton_summer = saanichton.loc[(saanichton["Month"] > 9) & (saanichton["Month"] < 12)]
saanichton_summer["Winter_Precip"] = saanichton_summer.groupby("Year").P.transform('mean')
saanichton_trimmed = saanichton_summer[saanichton_summer["Month"] == 10]

print(saanichton_trimmed)

# # regression function

d = np.polyfit(saanichton_trimmed['Year'],saanichton_trimmed["Winter_Precip"],1)
f = np.poly1d(d)
saanichton_trimmed.insert(6,'Regression Line',f(saanichton_trimmed['Year']))
ax = saanichton_trimmed.plot(x ='Year',y="Winter_Precip")
...
plt.title("'October-November' Precipitation in Saanichton, BC (1917-2017)")
plt.ylabel("October-November Mean Precipitation (mm)")
